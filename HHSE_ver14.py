...
def main(conn):
    ...
    student_id = student_data.number_input("Student ID")
    ...
    physician_phone_number = student_data.text_input("Physician Phone Number")
    ...
    if submit_button:
        ...
        try:
            if not physician_phone_number or not is_valid_number(parse(physician_phone_number, None)):
                missing_fields.append("Physician Phone Number (must be a valid phone number)")
        except NumberParseException:
            missing_fields.append("Physician Phone Number (must be a valid phone number including country code)")
        ...
        if missing_fields:
            ...
            insert_student(conn, (
                student_id, student_name, student_dob, student_address, school_district, physician_name,
    ...
    query_field = query_form.selectbox("Select Field", options=['Student_ID', 'Student_Name', 'Student_DOB', 'Student_Address', 'School_District', 'Physician_Name', 'Physician_Phone_Number', 'Type_Of_Authorizer', 'License_Num', 'Physician_Address', 'Home_Hospital_Both', 'Consecutive_Recurring_Basis', 'Admittance_Date', 'Regular_Reduced_Workload', 'Return_Date'])
...
def insert_student(conn, student_data):
    ...
    query = '''INSERT INTO students (
                Student_ID, Student_Name, Student_DOB, Student_Address, School_District, 
